...
class Setting_Windown(QMainWindow):
    ...
    def SaveData(self):
        ...
        conn.commit()
        self.Measure()
        conn.execute("INSERT INTO  Measure (Area, Distance) VALUES (?,?);",
    ...
    def ProcessImage(self,orginalImage,index,x,y,width,height):
      try:
        ...
        #self.thresh = cv2.cvtColor(HsvColor, cv2.COLOR_BGR2GRAY)
    ...
    def GetResult(self):
      ...
      if len(self.items)==0:
        ...
        if self.ui.radioButton.isChecked():  # kiem tra Area co duowc chon khong
          for i in range(len(self.items)):
            if self.items[i][0].isSelected():
        ...
        if self.ui.radioButton_2.isChecked():
          ...
          for i in range(len(self.items)):
              if self.items[i][0].isSelected():
                  hight = self.Dis_hight(i)
                  if hight:
                      self.ui.label_Result_test.setText("OK One")
                      self.ui.label_Result_Area.setText(str(self.Distance_h))
                      return True
                  else:
                      self.ui.label_Result_test.setText("NG One")
                      self.ui.label_Result_Area.setText(str(self.Distance_h))
    ...
    def Dis_hight(self,dhight):
        ...
        for i in range (len(contour)):
              for j in range (len(mang_x)):
                if contour[i][0][0] == mang_x[j]:
                   height_x.append(contour[i])
        ...
        for i7 in range (len(height_xm)):
              self.pen = QPen(QColor(Qt.yellow))
              self.line = QGraphicsLineItem(height_xm[i7][0][0][0]+self.x,height_xm[i7][0][0][1]+self.y, height_xm[i7][1][0][0]+self.x,height_xm[i7][1][0][1]+self.y)
              ...
              self.line1 = QGraphicsLineItem(x_div1, y_div,
                                             height_xm[i7][1][0][0] + self.x, height_xm[i7][1][0][1] + self.y)
              self.line1.setZValue(2)
              self.line1.setPen(self.pen1)
              self.scene.addItem(self.line1)

              self.pen2 = QPen(QColor(Qt.blue))
              self.line2 = QGraphicsLineItem(x_div2, y_div,
                                             height_xm[i7][1][0][0] + self.x, height_xm[i7][1][0][1] + self.y)
              ...
              self.line3 = QGraphicsLineItem(x_divd1, y_divd,
                                             height_xm[i7][0][0][0] + self.x, height_xm[i7][0][0][1] + self.y)
              self.line3.setZValue(2)
              self.line3.setPen(self.pen3)
              self.scene.addItem(self.line3)

              self.pen4 = QPen(QColor(Qt.blue))
              self.line4 = QGraphicsLineItem(x_divd2, y_divd,
                                             height_xm[i7][0][0][0] + self.x, height_xm[i7][0][0][1] + self.y)
              self.line4.setZValue(2)
              self.line4.setPen(self.pen3)
              self.scene.addItem(self.line4)
              self.Distance_h = dist.euclidean((height_xm[i7][0][0][0],height_xm[i7][0][0][1]),(height_xm[i7][1][0][0],height_xm[i7][1][0][1]))
              if self.min_h<=self.Distance_h<=self.max_h:
                  return True
              else:
                  return False
    def setValueZ(self):
